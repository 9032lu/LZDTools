...
#define LZDWeakSelf __weak typeof(self) weakSelf = self;

@interface LZDLoopView () <UIScrollViewDelegate>
//轮播的图片数组
    ...
    for (int i = 0; i < imageArray.count; i++) {
        if ([imageArray[i] isKindOfClass:[UIImage class]]) {
            ...
            if (_placeholderImage) {
                ...
                UIImage *placeholderImage = [UIImage imageNamed:@"banner_bg"];
...
- (void)setPagePosition:(PageControlPosition)pagePosition {
    ...
    if (self.type != 2) {
        _pageControl.frame = CGRectMake(self.width - HORMARGIN - size.width, self.height-20, size.width, size.height);
    }else{
        _pageControl.frame = CGRectMake((self.width - size.width)/2, self.height-20, size.width, size.height);
    }
...
- (void)startTimer {
    ...
    ///借书卡轮播图
    if (self.cardViewType == CardViewScroll) {
...
- (void)layoutSubviews {
    ...
    UIView *bgView = [[UIView alloc] initWithFrame:CGRectMake(0, self.height - DES_LABEL_H-15, self.width, 35)];
    ...
    _describeLabel.frame = CGRectMake(15, self.height - 35/2 -DES_LABEL_H / 2  , self.width / 2, DES_LABEL_H);
    [self addSubview:self.describeLabel];
    ///借书卡轮播图
    if (self.cardViewType == CardViewScroll) {
        bgView.hidden = YES;
...
- (void)imageClick {
    ...
    } else if ([self.lzdLoopDelegate respondsToSelector:@selector(loopView:clickImageAtIndex:)]){
        [self.lzdLoopDelegate loopView:self clickImageAtIndex:self.currIndex];
    }
}
